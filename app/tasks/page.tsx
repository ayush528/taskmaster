    ...
    <DashboardLayout title="">
      ...
      <div className="space-y-4 mb-12">
        {pendingTasks.map(task => (
          <div key={task.id} className="group flex items-center justify-between rounded-2xl border border-slate-100 bg-white p-5 shadow-sm transition-all hover:border-slate-300">
      ...
      {completedTasks.length > 0 && (
        <div className="space-y-4">
          {completedTasks.map(task => (
            <div key={task.id} className="group flex items-center justify-between rounded-2xl border border-transparent bg-transparent p-5 transition-all hover:bg-white hover:shadow-sm hover:border-slate-100 opacity-70">
              <div className="flex items-start space-x-5 relative w-full">
                <div className="pt-0.5">
                  <button 
                    onClick={() => toggleTaskCompletion(task.id)}
                ...
                <div className="flex-1">
                  <h3 className="font-semibold text-slate-500 text-base line-through">{task.title}</h3>
