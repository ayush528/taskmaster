...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      await authAPI.register(email, password, name, Intl.DateTimeFormat().resolvedOptions().timeZone);
        ...
        <div className="w-full max-w-[500px] rounded-2xl bg-white p-8 shadow-sm border border-slate-100">
          ...
          <form onSubmit={handleRegister} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="flex w-full items-center justify-center rounded-xl bg-blue-600 py-2.5 px-4 text-sm font-medium text-white shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-600 focus:ring-offset-2 transition-colors mt-4 disabled:opacity-70 disabled:cursor-not-allowed"
            >
